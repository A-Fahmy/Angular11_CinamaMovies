...
export class AuthService {
  ...
  GetRoleName(email: string) {
    return this.http.get('http://localhost:53476/Account/GetRoleName/' + email, { responseType: 'text' }).pipe();
  }

  ValidateUser(email: string, role: string) {
    return this.http.get('http://localhost:53476/Account/CheckUserClaims/' + email + '&' + role,
